...
for (let i = 0; i < data.length; i++) {
  const row = data[i];
  max = Math.max(max, visit([[0, i, 1, 0]], new Set()));
  max = Math.max(max, visit([[row.length - 1, i, -1, 0]], new Set()));
}

for (let i = 0; i < data[0].length; i++) {
  max = Math.max(max, visit([[i, 0, 0, 1]], new Set()));
  max = Math.max(max, visit([[i, data.length - 1, 0, -1]], new Set()));
}
